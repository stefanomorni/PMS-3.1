...
let
    ...
    import_new_ids = Table.NestedJoin(errors_to_null, {"smo_id"}, new_ids_tbl, {"smo_id"}, "new_ids", JoinKind.LeftOuter),
    expand_new_ids = Table.ExpandTableColumn(import_new_ids, "new_ids", {"lukb_id"}, {"lukb_id"}),
    smo_lukb_ids = Table.CombineColumns(expand_new_ids,{"LUKB_id", "lukb_id"},Combiner.CombineTextByDelimiter("", QuoteStyle.None),"LUKB_id")
