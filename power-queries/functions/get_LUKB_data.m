...
let
    Data = (SearchTerm as text, CurrencyCode as text, Fields as text, optional AssetClass as nullable text, optional ExchangeCode as nullable text) => 
    let
        ...
        Datetime_To_DateTime = if Exch_str = "" 
                                then Table.TransformColumnTypes(remove_status_col_tbl,{{"LVAL:datetime", type datetime}})
                                else remove_status_col_tbl,
        Sort_By_Datetime = if Exch_str = "" 
                            then Table.Sort(Datetime_To_DateTime,{{"LVAL:datetime", Order.Descending}})
                            else Datetime_To_DateTime,
        Kept_Most_Current = if Exch_str = "" 
                            then Table.FirstN(Sort_By_Datetime,1)
                            else Sort_By_Datetime     
    in
        Kept_Most_Current
in
    Data
