...
let
        Data = (securityIDs as list, Fields as nullable list, StartURL as nullable text) =>
            let
                ...
                pull_securityIDs = Json.FromValue(
                    List.Transform(securityIDs, each [id = Text.From(_), selected = "true"])
                ),
                pull_fields =
                    if Fields <> null then
                        Json.FromValue(List.Transform(Fields, each [id = Text.From(_), selected = "true"]))
                    else
                        Json.FromValue(List.Transform(BASIC_FIELDS, each [id = _, selected = "true"])),
                URL =
                    if StartURL <> null then
                        Text.Trim(StartURL)
                    else
                        "https://boersenundmaerkte.lukb.ch/lukb/details/1200526,847,1#Tab0",
