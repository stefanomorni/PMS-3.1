...
(current_wb_range_name as text, optional column_name as nullable text, optional row_first_column_value as nullable text) =>
let
    // Determine the range type based on input parameters
    range_type = 
        if column_name = null and row_first_column_value = null then "named_range"
        else if column_name = null then
            if row_first_column_value = "Headers" then "table_headers"
            else "table_row"
        else if row_first_column_value = null then "table_column"
        else "cell",
    ...
    range_values_list = 
        if range_type = "named_range" then 
            if Table.ColumnCount (excel_table) > Table.RowCount(excel_table)
                then Record.ToList(excel_table{0})
                else excel_table[Column1]
            ...
            in
                if row_index = -1 then null else Record.ToList(excel_table{row_index})
            ...
            in
                if row_index = -1 then null else {table_column{row_index}}
        else null
in
    range_values_list
