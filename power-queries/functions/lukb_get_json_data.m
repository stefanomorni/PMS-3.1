...
let
    get_json_data = (
        ...
        let
            ...
            list_id = if in_customer_id is null then "BFE028A242BD0691902C313CEFA247C9" else in_customer_id,
            pull_id = if in_pull_id is null then lukb_get_pull_id(list_id) else in_pull_id,
            referer = "https://boersenundmaerkte.lukb.ch/lukb/lists/CUSTOMER?q=" & list_id & "&t=list",
            timeout = if in_timeout_seconds = null then
                #duration(0, 0, 0, 15)
                // set to 15 seconds; default is 100 seconds
            else
                #duration(0, 0, 0, in_timeout_seconds),
            // 3. Calculate delay for retry
            base_delay = 1,
            // Base delay in seconds
            exponential_factor = 1,
            // Factor for exponential backoff
            jitter = Number.RandomBetween(0, 500) / 1000,
            ...
            delayed_request =
                if current_try > 0 then
                    Function.InvokeAfter(
                        () => Web.Contents(updatePull_url, options), #duration(0, 0, 0, retry_delay)
                    )
                else
                    Web.Contents(updatePull_url, options),
            ...
            // 7. Retry logic with safety cap
            effective_max_retries = if max_retries > 5 then 5 else max_retries,
            current_retry = current_try + 1,
            valid_json =
                if current_retry <= effective_max_retries then
                    ...
                    error "LUKB_TIMEOUT: No valid JSON data found after retries. Fail-fast safety triggered."
        in
            valid_json
in
    get_json_data
