...
let
        smo_id = "DE000A161408:EUR",
        primary_exchange = null,
        // Function parameters example
        fields_types = {"M_CUR:value", "M_NAME:value", "MARKET:value", "LVAL:datetime", "PMTURNOVER:value"},
        ...
        is_primary_exchange = if primary_exchange = true or Text.Lower(exchange_code) = "p" then true else false,
        // get the corresponding lukb ids for currency and exchange if provided
        CurrencyCode =
            if currency_code = null then
                currency_code
            else
                try
                    Table.SelectRows(lukb_currencies_tbl, each ([cur_code] = currency_code)){0}[cur_id]
                otherwise
                    "Error: the currency code was not found",
        AssetClass = null,
        ExchangeCode =
            if exchange_code = null or Text.Length(exchange_code) = 1 then
                null
            else
                try
                    Table.SelectRows(lukb_exchanges_tbl, each ([exch_code] = exchange_code)){0}[exch_id]
                otherwise
                    "Error: the exchange code was not found",
        ...
        Filter_exchange =
            if Exch_str <> "" then
                Table.SelectRows(data_fields, each ([#"M_CUR:value"] = Currency_str and [#"MARKET:value"] = Exch_str))
            else
                Table.SelectRows(data_fields, each ([#"M_CUR:value"] = Currency_str)),
        ...
        replace_invalid_values = Table.ReplaceValue(
            remove_status_col_tbl, "-", null, Replacer.ReplaceValue, {"PMTURNOVER", "LVAL:datetime"}
        ),
        removed_tsd_separator = Table.ReplaceValue(
            replace_invalid_values, "'", "", Replacer.ReplaceText, {"PMTURNOVER"}
        ),
        ...
        datetime_to_datetime =
            if Exch_str = "" then
                Table.TransformColumnTypes(final_table, {{"LVAL:datetime", type datetime}})
            else
                final_table,
        sort_by_datetime =
            if Exch_str = "" then
                ...
                final_table,
        most_current = if Exch_str = "" then Table.FirstN(sort_by_datetime, 1) else final_table,
        sort_by_turnover =
            if Exch_str = "" then
                Table.Sort(datetime_to_datetime, {{"PMTURNOVER", Order.Descending}})
            else
                final_table,
        highest_turnover = if Exch_str = "" then Table.FirstN(sort_by_turnover, 1) else final_table,
        result = if is_primary_exchange = true then highest_turnover else most_current,
