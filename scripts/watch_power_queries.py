...
def check_and_auto_split():
    ...
    if canon_mtime > sentl_mtime:
        ...
        if run_script(SPLIT_SCRIPT, "split"):
            ...
            print("[startup] Auto-split FAILED.")
    else:
...
def main():
    ...
    project_name = (
        project_config.get_project_name(config)
        if "project_config" in sys.modules
        else "Power Query"
    )

    print("=" * 60)
    print(f" {project_name} - Power Query Watcher")
    print("=" * 60)
